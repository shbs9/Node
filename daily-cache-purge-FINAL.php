...
class Daily_Cache_Purge {
    ...
    public static function execute_purge($triggered_by = 'wp-cron') {
        // Mark that we're running from a legitimate cron execution
        if (!defined('WP_CLI_CRON_RUNNING')) {
            define('WP_CLI_CRON_RUNNING', true);
        }
    ...
    private static function purge_cache() {
        ...
        // CRITICAL: If we're already running inside WP-CLI context during a purge subprocess,
        // don't call WP-CLI again (prevents infinite loop)
        if (defined('WP_CLI') && WP_CLI && !defined('WP_CLI_CRON_RUNNING')) {
            // We're in WP-CLI but NOT from a legitimate cron run
            // This happens when WP-CLI loads WordPress as part of executing our shell command
            return ['success' => false, 'output' => '', 'error' => 'Skipped to prevent WP-CLI recursion'];
        }
    ...
    private static function log_purge_event($status, $output, $error, $execution_time, $triggered_by = 'wp-cron') {
        ...
        if ($result === false) {
            error_log('[Cache Purge] Failed to write to log file: ' . $log_file);
        }
